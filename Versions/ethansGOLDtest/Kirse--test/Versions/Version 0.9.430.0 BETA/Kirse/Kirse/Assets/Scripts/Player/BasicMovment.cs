...
public class BasicMovment : MonoBehaviour
{
    ...
    void Update()
    {
        if (Time.timeScale > 0)
        {
            ...
            //{
            //    Debug.Log("Enemy" + i);
            //    enemiesToDamage[i].GetComponentInChildren<enemyAI>().takeDamage(1);
            //    if (enemiesToDamage[i].GetComponentInChildren<enemyAI>().health <= 0)
            //    {
    ...
    IEnumerator Example(Vector3 direction, Collider2D coll)
    {
        ...
        gameObject.layer = 9; //Dash layer for staying at walls
        hitBox.tag = "Dash";
